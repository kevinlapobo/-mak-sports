<div>
    ...
    <div style="background:#fff; border-radius:12px; padding:24px; margin-bottom:20px; border:1px solid #e5e7eb;">
        <div style="display:flex; gap:20px; align-items:center;">
            <div style="width:80px; height:80px; background:#006633; border-radius:50%; display:flex; align-items:center; justify-content:center; color:#fff; font-size:32px; font-weight:800; flex-shrink:0;">
                {{ strtoupper(substr($player->name, 0, 1)) }}
            </div>
            <div style="flex:1;">
                <h1 style="font-size:24px; font-weight:800; color:#004d26; margin-bottom:4px;">{{ $player->name }}</h1>
                @if($player->position)
                    <div style="font-size:16px; color:#006633; font-weight:700; margin-bottom:4px;">{{ $player->position }}</div>
                @endif
                @if($player->team)
                    <div style="font-size:14px; color:#9a9a9a;">
                        Team: <a href="{{ route('team.detail', $player->team->slug) }}" style="color:#006633; font-weight:700; text-decoration:none;">{{ $player->team->name }}</a>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(120px, 1fr)); gap:12px; margin-top:20px; padding-top:20px; border-top:1px solid #f0f0f0;">
                @if($player->jersey_number)
                    <div style="text-align:center; background:#f9fafb; border-radius:10px; padding:14px;">
                        <div style="font-size:24px; font-weight:800; color:#006633;">{{ $player->jersey_number }}</div>
    ...
    <div style="background:#fff; border-radius:12px; padding:20px; border:1px solid #e5e7eb;">
        <h3 style="font-size:16px; font-weight:800; color:#004d26; margin-bottom:16px;">Statistics</h3>
