<div>
    ...
    @if($sports->count() > 0)
        <div style="margin-bottom:16px; display:flex; gap:10px; flex-wrap:wrap;">
            <a href="{{ route('standings') }}"
               style="background:{{ !$sport ? 'var(--muk-green)' : '#fff' }}; color:{{ !$sport ? '#fff' : '#111' }}; border:1.5px solid #e5e7eb; border-radius:10px; padding:8px 14px; text-decoration:none; font-size:12px; font-weight:700;">
                All
            </a>
            @foreach($sports as $s)
                <a href="{{ route('standings', ['sport' => $s->slug]) }}"
                   style="background:{{ $sport === $s->slug ? 'var(--muk-green)' : '#fff' }}; color:{{ $sport === $s->slug ? '#fff' : '#111' }}; border:1.5px solid #e5e7eb; border-radius:10px; padding:8px 14px; text-decoration:none; font-size:12px; font-weight:700;">
                    {{ $s->icon }} {{ $s->name }}
                </a>
            @endforeach
        </div>
    ...
    @if($standings->count() > 0 && $competition)
        <div style="background:#fff; border-radius:12px; overflow:hidden; border:1px solid #e5e7eb;">
            {{-- Competition Header --}}
            <div style="background:var(--muk-green); color:#fff; padding:14px 18px;">
            ...
            {{-- Table Header --}}
            <div style="display:grid; grid-template-columns:40px 1fr repeat(7, 50px); gap:4px; padding:10px 14px; background:#f9fafb; border-bottom:1px solid #e5e7eb; font-size:11px; font-weight:700; color:var(--muk-green-dark); text-align:center;">
                <div>#</div>
            ...
            @foreach($standings as $index => $standing)
                <div style="display:grid; grid-template-columns:40px 1fr repeat(7, 50px); gap:4px; padding:10px 14px; border-bottom:1px solid #f0f0f0; font-size:13px; text-align:center; align-items:center; {{ $index < 2 ? 'background:#f0faf4;' : '' }}"
                    ...
                    <div style="font-weight:800; color:var(--muk-green-dark);">{{ $index + 1 }}</div>
                    ...
                    <div style="color:{{ ($standing->goal_difference ?? 0) >= 0 ? 'var(--muk-green)' : 'var(--muk-red)' }}; font-weight:700;">
                        {{ ($standing->goal_difference ?? 0) > 0 ? '+' : '' }}{{ $standing->goal_difference ?? 0 }}
                    </div>
                    <div style="font-weight:800; color:var(--muk-green-dark);">{{ $standing->points ?? 0 }}</div>
                </div>
            @endforeach
        </div>
    @else
